...
# Compose interpolates ${WEB_PUBLISH_PORT} from --env-file AND project-root .env.
# Bare `docker compose up` without --env-file used to default to 8088 and collide
# with host nginx (IoT). Keep a tiny .env so the publish port always matches.
PORT="$(grep -E '^WEB_PUBLISH_PORT=' "$ENV_FILE" | cut -d= -f2- | tr -d '\r' || true)"
PORT="${PORT:-8090}"
cat > .env <<EOF
# Generated by aliyun-deploy.sh — do not commit. Ports only (secrets stay in $ENV_FILE).
WEB_PUBLISH_PORT=${PORT}
EOF

if command -v ss >/dev/null 2>&1; then
  if ss -tln | grep -qE ":${PORT}\\b" && ! docker ps --format '{{.Ports}}' | grep -q ":${PORT}->"; then
    echo "Host port ${PORT} is already in use (not by ai-tp-web). Set WEB_PUBLISH_PORT in ${ENV_FILE}." >&2
...
if ! docker ps --format '{{.Names}}' | grep -qx ai-tp-web-1; then
  echo "==> Recreating web (previous bind likely used a busy port)..."
