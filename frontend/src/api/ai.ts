...
export const agentWorkflowApi = {
  create: (projectId: number, body: { requirement_text: string; [key: string]: unknown }) => req<Record<string, unknown>>(`/projects/${projectId}/agent-workflows`, { method: "POST", body: JSON.stringify(body) }),
  list: (projectId: number) => req<Array<Record<string, any>>>(`/projects/${projectId}/agent-workflows`),
