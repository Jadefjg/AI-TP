...
const queryHandoff = computed(() => buildPipelineQuery(props.handoff || {}));

/** Prefer live route so highlight never drifts from the page actually shown. */
const activeKey = computed<AiPipelineStepKey>(() => {
  const fromRoute = pipelineStepFromRoute(route);
  if (fromRoute) return fromRoute.key;
  return props.current || "requirements";
});

const currentIndex = computed(() =>
  AI_PIPELINE_STEPS.findIndex((step) => step.key === activeKey.value),
);

const go = (step: AiPipelineStep) => {
  void router.push({
    path: step.path,
    query: queryHandoff.value,
  });
};
</script>

<template>
  <nav class="ai-pipeline" aria-label="AI 智能流水">
    <div class="ai-pipeline__row">
      <button
        v-for="(step, index) in AI_PIPELINE_STEPS"
        :key="step.key"
        type="button"
